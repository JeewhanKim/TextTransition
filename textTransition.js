...
const initialStyles = () => {
  if(!transitionSection.length) return
  galleries   = transitionSection.find('.pictures')

  if(initialState) {
    galleries.find('.caption').each((i, caption) => { captions.push($(caption).outerHeight()) })
    galleries.css('background-position', `center top`)
    transitionSection.each((i, section) => {$(section).css('z-index', galleries.length - i )})
...
if(jQuery !== undefined) {
  jQuery.prototype.textTransition = function(options){
    init(this, options);
  }
} else {
  console.warn('TexTransition Error: jQuery is not imported');
}